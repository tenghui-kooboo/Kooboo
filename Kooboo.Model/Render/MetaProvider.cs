...
namespace Kooboo.Model.Render
{
    public class ApiMetaProvider : IApiMetaProvider
    {
        public ApiInfo GetMeta(string url)
        {
            ...
            var usernameUniqueRule = new ValidationRules.UniqueRule("/user/IsUniqueName?name={userName}", "username is not unique");
            var passwordSameasRule = new ValidationRules.SameAsRule("password", "password is not the same.");
            var emailUnqiueRule = new ValidationRules.UniqueRule("/user/IsUniqueEmail?email={email}", "email is not unique");
                    ...
                    return new ApiInfo
                    {
                        Model = new ModelInfo
                        {
                            Properties = new List<PropertyInfo>
                                {
                                    new PropertyInfo { Name = "UserName", Rules = new List<ValidationRules.ValidationRule> { requiredRule,usernameUniqueRule }, Type = typeof(String) },
                                    new PropertyInfo { Name = "Password", Rules = new List<ValidationRules.ValidationRule>(){ requiredRule}, Type = typeof(string) },
                                    new PropertyInfo { Name = "ConfirmPassword", Rules = new List<ValidationRules.ValidationRule>(){ requiredRule,passwordSameasRule}, Type = typeof(string) },
                                    new PropertyInfo { Name = "Email", Rules = new List<ValidationRules.ValidationRule>(){ requiredRule,emailUnqiueRule}, Type = typeof(string) },
                                },
                            Type = typeof(ModelInfo)
                        },
                        Parameters = new List<PropertyInfo>
                            {
                                new PropertyInfo { Name = "registerModel", Type = typeof(RegisterModel) }
                            }
                    };
                    break;
                case "/user/login":
